...
from propsde.graph_representation.word import NO_INDEX
...
class BratVisualizer:
    def __init__(self):
        self.visualize_html = open(BASE_PATH + './visualizations/visualize.html').read() 
    ...
    def get_attributes(self, uid, d):
        ret = []
        feats = d['feats']
        featList = ['tense', 'passive', 'negated', 'subjunctive']
        if not d['predicate']:
