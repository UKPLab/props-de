...
def merge_nodes(gr, node1, node2):
    if (gr.has_edge((node1, node2))):
        rel_type = gr.edge_label((node1, node2))
        gr.del_edge((node1, node2))
    else:
        rel_type = gr.edge_label((node2, node1))
...
def replace_head(gr, head, dependent):
    if head.uid in gr.nodesMap:
        dependent.original_text.extend(head.original_text)
        dependent.surface_form.extend(head.surface_form)
        # place children
        child_dict = head.neighbors()
        for child_rel in child_dict:
            for child in child_dict[child_rel]:
                if child != dependent:
                    gr.add_edge((dependent,child), child_rel)
        # add parents
        parent_dict = head.incidents()
        for parent_rel in parent_dict:
            for parent in parent_dict[parent_rel]:
                if parent != dependent:
...
def find_marker_idx(g, conj_gov, rel):
    used = set(reduce(lambda x,y: x+y,[[w.index for w in n.surface_form] for n in g.nodes()]))
    candidates = []
    for i in range(1,max(used)):
        if i not in used:
            candidates.append(i)
    if len(candidates) == 0:
        return max(used)+1
        ...
        conj_dep = conj_gov.neighbors()[rel][0]
        if conj_dep.minIndex() < conj_gov.minIndex():
            minIdx = conj_dep.maxIndex()
            maxIdx = conj_gov.minIndex()
        else:
            minIdx = conj_gov.maxIndex()
            maxIdx = conj_dep.minIndex()
        for c in candidates:
            if c > minIdx and c < maxIdx:
                return c
        return max(used)+1
