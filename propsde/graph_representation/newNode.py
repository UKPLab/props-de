from propsde.graph_representation.word import Word,NO_INDEX, strip_punctuations
import cgi
...
POSSESSIVE = "haben"   # the textual value of a possessive node
...
EXISTENSIAL = "existiert" # the textual value of a conjunction node
...
def join(node1,node2,gr):
    ...
    original_text = sorted(original_text, key= lambda w:w.index)
    surface_form = list(set(node1.surface_form).union(node2.surface_form))
    surface_form = sorted(surface_form, key= lambda w:w.index)
