...
from propsde.utils.utils import *
...
class DepTree(object):
    ...
    def __init__(self,pos,word,id,parent=None,parent_id = None,parent_relation=None,children=[],wsj_id = 0, sent_id = 0, lemma=None, morph=None):
        ...
        self.parent_relation = encode_german_characters(parent_relation)          # Node's parent relation
        self.parent_id = parent_id                          # Node's parent id
        self.pos = pos                                          # pos tag
        self.word = encode_german_characters(word)         # word from sentence
...
#TODO            print self.get_original_sentence(False)
    ...
    def to_original_format_for_json(self,root=False):
        ...
        ret = [(self.id,u"    ".join([unicode(x) for x in [self.id,self.word,UNDERSCORE,self.pos,self.pos,UNDERSCORE,parent_id,self.parent_relation,UNDERSCORE,UNDERSCORE]]))]
        for child in self.children:
    ...
    def to_original_format(self,root=False):
        ...
        def inner(depTree):
            ...
            ret = [(depTree.id,"\t".join([unicode(x) for x in [depTree.id,depTree.word,UNDERSCORE,depTree.pos,depTree.pos,UNDERSCORE,parent_id,depTree.parent_relation,UNDERSCORE,UNDERSCORE]])+"\n")]
...
#REMOVED CODE FOR LOOKING AT DETERMINERS - 
        ...
        if isinstance(det,dict) and "Value" in det: 
            if det["Value"].lower() in definite_determiners:
                return definite_label
        # b) determining possesive
        for child in self.children:
            if child.get_parent_relation() == "AG":
                return definite_label
    ...
    def _VERBAL_PREDICATE_FEATURE_Determiner(self):
        ...
        span=0
        # check if one of dependents causes the negation of this predicate
        # DE: additional check for pos tag
        det_nodes = filter(lambda x:x.get_parent_relation() in determiner_dependencies and x.pos in determiner_pos, self.children)
        if det_nodes:
    ...
    def _VERBAL_PREDICATE_FEATURE_Modal(self):
        ...
        #else:
        #    return {"Value":ret}
        if self.get_parent_relation() == 'OC' and self.parent.pos in VERB_POS_MOD:
            return {"Value":[self.parent.word]}
        else:
            return False
            
    # subjunctive according to morphological analysis
    def _VERBAL_PREDICATE_FEATURE_Subjunctive(self):
        if self.morph and 'subj' in self.morph:
            return subjunctive_label
        else:
            return False
    ...
    def _VERBAL_PREDICATE_FEATURE_Tense(self):
        ...
        if not self.is_predicate():
            return False
        from pattern.de import tenses
        try:
            ts = [t[0] for t in tenses(self.word)]
            if ts[0] in ['past','future','present']: # could also be "infinitive"
                return ts[0]
            else:
                return False
        except:
    ...
    def _VERBAL_PREDICATE_FEATURE_Lemma(self):
            ...
            #return lemma(self.word_original)
        # for all verbs (mapping not needed), except VVPPs in copular constructions (zustandspassiv)
        if self.pos in pos_penn_to_wordnet and not ( self.pos == "VVPP" and self.get_parent_relation() == "PD" ):
            return self.lemma
        else:
            return False
